...
manager.on('shardCreate', shard => {
    ...
    if (wb) {
        ...
        shard.on("reconnecting", () => {
            wb.send({ embeds: [new EmbedBuilder().setDescription(`**<:online:1136224896461045910> \`#${shard.id}\` - Bot yeniden başlatılıyor**`).setColor("Green")] });
        });
        shard.on("ready", async () => {
            wb.send({ embeds: [new EmbedBuilder().setDescription(`**<:bosta:1136225253874479134> \`#${shard.id}\` - Bot Açıldı**`).setColor("Yellow")] });
        });
        shard.on("death", () => {
            wb.send({ embeds: [new EmbedBuilder().setDescription(`**<:offline:1136224802340880484> \`#${shard.id}\` - ID'li shardın bağlantısı koptu, yeniden başlatılmayı deniyor**`).setColor("Red")] });
        });
...
manager.spawn().then(() => {
    if (wb) wb.send({ embeds: [new EmbedBuilder().setDescription(`**Bot başarıyla başlatıldı ve kullanıma hazır**`).setColor("DarkPurple")] });
